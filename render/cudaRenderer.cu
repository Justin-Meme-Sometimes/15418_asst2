...
__global__ void kernelAdvanceFireWorks() {
  ...
  // The array indices index3i and index3k assume valid memory access without
  // boundary access. (maybe out of bounds?)
...
__device__ __inline__ void shadePixel(float2 pixelCenter, float3 p,
  ...
  // Ethan: done this suggestion by storeing conditionals in isSnow, not sure if
  // made better tho
  if (isSnow) {
...
void CudaRenderer::loadScene(SceneName scene) {
  sceneName = scene;
  loadCircleScene(sceneName, numberOfCircles, position, velocity, color,
                  radius);
}
...
void CudaRenderer::setup() {
  ...
  //
  cudaMemcpy(cudaDevicePosition, position, sizeof(float) * 3 * numberOfCircles,
             cudaMemcpyHostToDevice);
  cudaMemcpy(cudaDeviceVelocity, velocity, sizeof(float) * 3 * numberOfCircles,
             cudaMemcpyHostToDevice);
